...
import { Bell, Search, ChevronDown, Download, Play, Filter, Menu, X, TrendingUp, AlertTriangle, CheckCircle, Loader2, History } from 'lucide-react';
...
export function CTSVDashboard({ onLogout }: CTSVDashboardProps) {
  ...
  const [lastSync, setLastSync] = useState<string>('2025-11-26 08:00');
  ...
  useEffect(() => {
    const fetchRiskStudents = async () => {
      setIsLoading(true);
      try {
        const response = await riskAssessmentApi.getAtRiskStudents();
        if (response.data) {
          const transformed = transformApiData(response.data.students);
  ...
  const runDetection = async () => {
    ...
    try {
      ...
      const response = await riskAssessmentApi.runDetection({ threshold: 60 });
      if (response.data) {
        const transformed = transformApiData(response.data);
        setStudents(transformed);
        setFilteredStudents(transformed);
        setLastSync(new Date().toLocaleString('vi-VN'));
        setNotifications(prev => prev + transformed.length);
      }
    } catch (err) {
      ...
      <div className="pt-[60px] flex">
        ...
        >
          ...
          <div className="p-4 border-t border-blue-200">
            <button className="w-full px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-indigo-600 text-white rounded-lg flex items-center justify-center gap-2">
